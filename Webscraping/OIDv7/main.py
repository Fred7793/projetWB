...
class ExtractData:
    def __init__(self, file_path, data_type, params, limit=None):
    ...
    def Download(self):
        ...
        for line in lines:
            ...
            if (class_name in class_names) and ismatched:
                img_ids.add(img_id)
                if limit and len(img_ids) >= limit:
                    break
...
if __name__ == "__main__":
    ...
    parser.add_argument('--limit', type=int, default=None, help='Limit the number of images to download')
    ...
    ExtractData("oidv6-train.csv", 'train', args, args['limit']).Download()
    ExtractData("oidv7-validation.csv", 'validation', args, args['limit']).Download()
    ExtractData("oidv7-test.csv", 'test', args, args['limit']).Download()